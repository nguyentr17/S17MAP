...
```{r}
female <- gender2[gender2$gender == 0,]$TimeUsedSec
male <- gender2[gender2$gender == 1,]$TimeUsedSec
pvl2 <- round(t.test(female, male)$p.value, digits = 3)
boxplot(TimeUsedSec ~ gender,data=gender2, main=paste("n=", dim(gender2)[1], ", two sided pval=", pvl2), xlab="Gender (1=Male, 0=Female)", ylab="Time Used Seconds")
```

We can further conduct a one-sided t-test to see whether male time used are significantly more than female time used or otherwise.
In this case, our Null Hypothesis is that there is no difference between the female and male time used. Our Alternative Hypothesis is that female use less time then male. The p-value is given as follows:

```{r}
pvl1 <- round(t.test(female, male, alternative="less")$p.value, digits = 3)
pvl1
```

Choose alpha = 0.05 for the one-sided t-test, since 0.021 is less than 0.05, we have evidence to reject the null hypothesis and confirm that in the entire gender group, female use less time than male.
